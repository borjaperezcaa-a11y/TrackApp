...
export const metadata = { title: "Facturas de cooperativa · TrackApp" };
...
export default async function FacturasExternasPage() {
  ...
  const { data } = await supabase
    ...
    .order("cobrada", { ascending: true })
    .order("fecha", { ascending: false });
  const rows = (data ?? []) as ExtRow[];

  const pendientes = rows.filter((r) => !r.cobrada);
  const cobradas = rows.filter((r) => r.cobrada);

  return (
    <>
      <PageHeader title="Cooperativa" kicker="Facturas registradas" fallbackHref="/facturas" actionHref="/facturas/externas/nueva" actionLabel="Registrar factura" />

      <Tabs />

      {rows.length === 0 ? (
        <div className="mt-8 text-center">
          <p className="text-[15px] font-semibold">Aún no has registrado facturas de cooperativa</p>
          <p className="mx-auto mt-1.5 max-w-[280px] text-[13px] text-dim">
            Son las que la coop emite en tu nombre. Hazles una foto y la IA rellena los datos, o adjunta el PDF.
          </p>
