-- ============================================================================
-- TrackApp · 0032_importe_total_sin_irpf.sql  (conformidad — FAQ AEAT nº20)
--   La FAQ de desarrolladores nº20 aclara que la retención de IRPF NO forma parte
--   del registro de facturación: el campo `ImporteTotal` (el que entra en la huella
--   y en el QR) es **base imponible + cuota de IVA repercutida**, SIN restar IRPF.
--   El IRPF afecta al "Total a pagar", que es otra cosa.
--
--   Hasta 0031, la función metía `v_total = base+IVA−IRPF` tanto en el campo
--   `total` de la factura como en el `ImporteTotal` de la huella y en el `importe`
--   del QR. Eso es correcto para el total a pagar, pero NO para el registro: al
--   cotejar/remitir, el importe del QR no cuadraría con el que calcula la AEAT.
--
--   Fix (★FAQ20): se separa `v_importe_total = base+IVA` (para huella y QR) del
--   `v_total = base+IVA−IRPF` (que se sigue guardando en la columna `total`, porque
--   es lo que el cliente paga y lo que muestra la factura). Idéntica a 0031 en todo
--   lo demás (suelo num_inicial ★C1, descripción ★D, validación de líneas ★M1).
--
--   NOTA: las funciones de rectificativa (0012/0013) tienen el mismo punto pendiente
--   (usan el total con IRPF en su huella/QR); se corregirán en su propio paso.
-- ============================================================================
...
begin
  ...
  if p_lines is not null then
    ...
    -- en la huella. Cada línea debe apuntar a un viaje SELECCIONADO y tener
  ...
  -- (de 0017) si el usuario fijó el número de su última factura. ★C1.
  ...
  v_total         := v_base + v_iva - v_irpf;   -- total a pagar (lo que ve el cliente)
  v_importe_total := v_base + v_iva;            -- ★FAQ20: ImporteTotal del registro
