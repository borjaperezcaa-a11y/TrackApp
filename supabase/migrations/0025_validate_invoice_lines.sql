-- ============================================================================
-- TrackApp · 0025_validate_invoice_lines.sql  (endurecimiento — auditoría M1)
--   La función de emisión acepta `p_lines` (precios editables del porte) tal
--   cual del cliente y con ellos calcula base/IVA/total y la HUELLA. Una llamada
--   directa a la RPC (está concedida a `authenticated`) podía emitir una factura
--   con líneas que no corresponden a los viajes seleccionados o con importes
--   negativos. Esto debilita la integridad que Veri*factu pretende garantizar.
--
--   Fix: al recibir `p_lines`, validar que CADA línea referencia un viaje de los
--   seleccionados (`p_trip_ids`, ya verificados como propios y pendientes) y que
--   cantidad/precio son válidos (cantidad > 0, precio ≥ 0). Se mantiene la
--   edición de precios; solo se rechazan líneas inventadas o importes imposibles.
--
--   Idéntica a 0010 salvo el bloque de validación marcado con ★M1.
-- ============================================================================
...
begin
  ...
  if p_lines is not null then
    ...
    -- ★M1: validar que las líneas no falsean los importes que entran en la huella.
    -- Cada línea debe apuntar a un viaje SELECCIONADO (ya validado como propio y
    -- pendiente) y tener cantidad > 0 y precio ≥ 0.
    if exists (
      select 1
