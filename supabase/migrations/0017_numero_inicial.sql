-- ============================================================================
-- TrackApp · 0017_numero_inicial.sql
-- Número de arranque al MIGRAR de facturación manual a la app: el usuario indica
-- el número de su última factura ya emitida fuera de la app y la numeración
-- continúa su serie (p. ej. última 42 → la app emite la 43).
--
-- Se implementa como un "suelo" en la función de emisión: el siguiente número
-- es greatest(max(num), num_inicial) + 1. Solo aplica a la (serie, año) para los
-- que se fijó, para no afectar a otras series ni al reinicio anual. Una vez hay
-- facturas emitidas, max(num) manda y el suelo es irrelevante (no crea huecos).
-- ============================================================================

alter table public.profiles
  add column if not exists num_inicial       int,        -- último nº emitido fuera de la app
  add column if not exists num_inicial_anio  smallint,   -- año al que aplica ese suelo
...
begin
  ...
  -- si el usuario fijó el número de su última factura para esta serie y año.
