-- ============================================================================
-- TrackApp · 0030_restaurar_suelo_num_inicial.sql  (fix QA — regresión C1)
--   La migración 0025 (endurecimiento ★M1) se rederivó de 0010 y, sin querer,
--   PERDIÓ el "suelo" de numeración que añadía 0017_numero_inicial.sql: el
--   siguiente número volvió a ser `max(num)+1` ignorando `num_inicial`.
--
--   Efecto: un autónomo que migra desde facturación manual e indica el número de
--   su última factura (p. ej. 42) veía "Próxima: FACT/26-43" en la UI, pero al
--   emitir obtenía FACT/26-01 — duplicando numeración ya emitida fuera de la app
--   y rompiendo el correlativo que la cadena Veri*factu pretende proteger.
--
--   Fix: recrear la función IDÉNTICA a 0025 (mantiene la validación ★M1 de
--   `p_lines`) volviendo a aplicar el suelo de 0017: el siguiente número es
--   greatest(max(num), num_inicial) + 1, solo para la (serie, año) en los que se
--   fijó. Una vez hay facturas emitidas, max(num) manda y el suelo es irrelevante.
-- ============================================================================
...
begin
  ...
  -- y año. ★C1: este bloque se había perdido en 0025; se restaura aquí.
